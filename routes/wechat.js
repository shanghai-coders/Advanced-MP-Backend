...
router.use('payment-callback', PaymentMiddleware(paymentConfig).getNotify().done( async (message, req, res, next) => {
  ...
  try {
    ...
    console.log(attach);
    const { id } = attach;
    const { data } = await axios.post(`${hostUrl}/order/update/${id}`, { status: 'paid'});
    console.log(data);
    // Update order status
    res.reply('success');
  } catch(e){
